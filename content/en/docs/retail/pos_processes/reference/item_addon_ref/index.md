...
| **Enabled** | If enabled, the item add-on feature is activated in the POS. This field will be activated as the last step when you finish the creation of features of the item add-on in the **Lines** section.  |
...
| **Use Unit Price** | The following options are available: <ul> <li> **Always** - whatever price is on the Item AddOn line, the system will use it to override the price on the sales line; </li> <li> **Non-Zero** - the price which is higher than zero on the add-on line will override the price on the sales line. Hence, if the price is zero in the add-on line, it will not be copied. </li> </ul> | 
