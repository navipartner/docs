...
3. In the tax-free handler selection field, select **GLOBALBLUE_I2** for the Global Blue integrated solution.
   As soon as you specify the handler, you need to set the accompanying parameters.
...
Tax-free vouchers can be printed out either as thermal or PDF, depending on the customer choice and coordination between them and Global Blue. For thermal vouchers, the **Output Path** field should direct to a thermal printer. For PDF vouchers, configure a supported PDF-capable printer or output destination in the target environment.
