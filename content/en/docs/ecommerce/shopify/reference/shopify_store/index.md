...
| **Customer No. (Price)** | Specifies the customer that will be used for calculating the prices of Shopify synchronization items. The system needs to identify parameters for item sales price and discount calculation, such as price and discount groups, VAT calculation parameters, etc. | 
| **No. of Prices per Request** | Specifies the number of item prices that can be sent to Shopify in a single batch price update request. The default value is 100, which will be used by the system if you set the field value to zero. | 
