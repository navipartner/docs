...
{{< alert icon="📝" text="Depending on your installed app version and setup, orders can be handled either through the classic <b>Import List</b> flow or through newer Shopify order-processing experience pages. The synchronization principles in this article apply to both flows."/>}}


1. The connection with the Shopify servers is established, and query for new, cancelled and closed orders is performed. The results are saved in the synchronization queue used by your setup (for example, the **Import List** in the classic flow).
...
2. New entries retrieved during the first process are processed, and consequently sales orders are updated (the new ones are created, the cancelled ones are deleted, and the closed ones are posted).
