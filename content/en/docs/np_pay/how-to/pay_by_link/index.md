...
3. Use the following fields and options to complete the setup. If necessary, refer to the screenshot below for more information.
   ...
   | **Pay by Link Expiration Duration** | Defines the expiration duration for Pay by Link payment requests. This determines for how long the payment link will remain valid after it has been generated and sent to the recipient. |
...
After authorization is completed, the **Payment Lines** are automatically populated with **Transaction ID**, **Amount**, and **Date Authorized**. Subsequently, the payment amount can either be captured automatically upon posting or triggered manually. During authorization, the corresponding amount is reserved on the customer's credit card, and that amount is deducted from the credit card balance after the payment is captured.
   ...
   | **Try Posting Count** | Specifies how many times the system retries posting if the initial posting attempt fails. |
